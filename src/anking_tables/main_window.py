...
class HtmlViewer(QWidget):
    ...
    #     # Get the updated HTML from the editor
    #     updated_html = self.htmlEditor.toPlainText()
    #
    #     # Update the card's field with the updated HTML
    #     self.card.note()[self.fieldName] = updated_html
    #     self.col.update_note(self.card.note())
    #
    ...
    #
    #     # print(f"Note: {note.id}", f"Card: {note_id}" f"Field Name: {field_name}", f"Updated HTML: {updated_html}")
    #     # print(f"Action: {col.update_note(note)}")
    #
    #     # Close the HtmlViewer window
    #     self.close()
    ...
    #         note = col.get_note(note_id)
    #
    #         logging.info(f"Before update: {note[field_name]}")
    #
    #         # Update the specified field in the note
    #         note[field_name] = updated_html
    #
    #         logging.info(f"After update: {note[field_name]}")
    #
    #         # Prepare the note data for the _update_notes method
    #         notes_to_update = [note]
    #
    #         # Call the _update_notes method to update the note in the database
    #         self._update_notes(notes_to_update)
    #
    #         logging.info(f"Note: {note.id}, Card: {note_id}, Field Name: {field_name}, Updated HTML: {updated_html}")
    #
